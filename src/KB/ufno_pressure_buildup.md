# U-FNO for Pressure Buildup in CO2 Storage

**Keywords**: ["PDE", "parabolic", "nonlinear", "forward-problem", "navier-stokes", "3D", "irregular", "dirichlet", "heterogeneous", "multi-scale", "turbulent", "FNO", "finite_difference", "U-Net", "adam", "relative-l2", "gpu", "pytorch"]

**Problem:** U-FNO (U-shaped Fourier Neural Operator) is an enhanced neural operator architecture for predicting pressure buildup dynamics in geological CO2 storage. It addresses the challenge of accurately forecasting pressure evolution in heterogeneous porous media with varying permeability, porosity, reservoir thickness, injection configurations, and flow properties. The problem involves solving coupled nonlinear PDEs for CO2-water multiphase flow over 30 years of injection, requiring accurate prediction of pressure fields with large radii of influence and sharp gradients near injection wells.

**Issues addressed:**
- **Large radius of influence**: Pressure buildup extends far from injection wells with smooth gradients. U-FNO's Fourier layers efficiently capture global pressure propagation patterns while U-Net layers handle sharp gradients near wells, maintaining accuracy even at distant locations where CNN struggles.
- **Sharp pressure gradients**: The hybrid architecture handles steep pressure changes near injection wells (up to 220 bar buildup) while maintaining accuracy in far-field regions, achieving 1.8x better accuracy than CNN for pressure front prediction.
- **Heterogeneous formations**: Accurately predicts pressure response in complex geological settings with permeability variations spanning 7 orders of magnitude (0.001 mD to 10 Darcy) and intricate anisotropy patterns.
- **Data efficiency**: Requires only 1/2.4 of the training data compared to CNN to achieve 1% mean relative error, saving 440 CPU hours in data generation.
- **Overfitting and generalization**: Fourier layers provide inherent regularization, with FNO-based models showing consistently smaller overfitting than CNN across training-testing gaps.
- **Multi-scale pressure dynamics**: Captures both global pressure diffusion (low-frequency, large-scale) and localized pressure responses to heterogeneity (high-frequency, small-scale).

## Key Method

U-FNO enhances the original Fourier Neural Operator (FNO) by introducing U-Fourier layers that combine:

1. **Fourier Layers** (first 3 layers): Perform integral kernel operations in Fourier space using Fast Fourier Transform, efficiently capturing global pressure propagation with mesh-free properties.

2. **U-Fourier Layers** (last 3 layers): Augment Fourier operations with U-Net convolutional paths to enrich representation of high-frequency pressure variations and sharp gradients.

3. **Hybrid Architecture Benefits**:
   - Fourier path: Provides resolution invariance, regularization, and efficient global pattern learning for large-scale pressure diffusion
   - U-Net path: Captures local convolution features, sharp pressure gradients near wells, and heterogeneity effects
   - Combined: Achieves 24% lower mean relative error than CNN while requiring less training data

The architecture processes 3D spatial-temporal inputs (96×200×24 grid with 12 input channels including kr, kz, porosity, injection location, injection rate, pressure, temperature, Swi, λ, grid_x, grid_y, grid_t) and outputs pressure buildup evolution over 24 time snapshots spanning 30 years.
...
- Output normalized to zero-mean, unit-variance
- Adam optimizer with step learning rate decay
- 140 epochs, batch size 4
...
class SimpleBlock3d(nn.Module):
    def __init__(self, modes1, modes2, modes3, width):
        ...
        # First 3 layers: Pure Fourier layers (capture global pressure diffusion)
        self.conv0 = SpectralConv3d(self.width, self.width, self.modes1, self.modes2, self.modes3)
        self.conv1 = SpectralConv3d(self.width, self.width, self.modes1, self.modes2, self.modes3)
        self.conv2 = SpectralConv3d(self.width, self.width, self.modes1, self.modes2, self.modes3)

        # Last 3 layers: U-Fourier layers (Fourier + U-Net for local pressure gradients)
        ...
        # U-Net components for last 3 layers (capture sharp pressure gradients)
    ...
    def forward(self, x):
        ...
        x3 = self.unet3(x)  # U-Net captures sharp pressure gradients
        x = x1 + x2 + x3
        x = F.relu(x)
        ...
        x = self.fc1(x)
        x = F.relu(x)
        x = self.fc2(x)  # Output: pressure buildup at each grid point and time
...
batch_size = 4
epochs = 140                 # Longer training for pressure (vs 100 for gas saturation)
learning_rate = 0.001
scheduler_step = 4           # LR decay every 4 epochs (vs 2 for gas saturation)
scheduler_gamma = 0.85       # LR decay factor (vs 0.9 for gas saturation)
...
# Helps capture sharp pressure gradients around injection well
for x, y in train_loader:
    ...
    # First derivative loss (helps with sharp gradients near well)
    der_loss = 0
    mask_dy = mask[:,:,:198,:]
    for i in range(batch_size):
...
- `modes1, modes2, modes3 = 10`: Number of Fourier modes retained in each spatial-temporal dimension. Captures essential pressure wave propagation patterns while filtering high-frequency noise.
- `width = 36`: Hidden channel dimension. Balances model capacity and computational cost.
- Number of layers: 3 Fourier layers + 3 U-Fourier layers. This 50/50 split optimally handles both global pressure diffusion and local pressure gradients.

**Training parameters**:
- `batch_size = 4`: Limited by GPU memory for 3D data
- `epochs = 140`: Longer training than gas saturation (100) due to smoother pressure fields requiring finer convergence
- `learning_rate = 0.001`: Initial learning rate
- `scheduler_step = 4, scheduler_gamma = 0.85`: More gradual learning rate decay than gas saturation (step=2, gamma=0.9)
- `weight_decay = 1e-4`: L2 regularization on model weights
- `derivative_weight = 0.5`: Weight on first derivative loss term

**Loss function parameters**:
- Relative L2 loss provides scale-invariance for pressure buildups ranging from ~20 to ~220 bar
- First derivative regularization weight = 0.5 improves sharp gradient prediction near injection wells
- Active cell masking accommodates variable reservoir thickness
- Output normalized to zero-mean, unit-variance (unlike gas saturation which stays in [0,1])
...
**Performance metrics**:
- Mean Relative Error (MRE): 0.68% (U-FNO) vs 0.89% (CNN) - 24% improvement
- R² score: 0.992 (U-FNO) vs 0.987 (CNN)
- Pressure front error: 12.0% (U-FNO) vs 21.2% (CNN) - 1.8x more accurate
- 6×10⁴ speedup compared to numerical simulation (10 minutes vs 0.01 seconds)
